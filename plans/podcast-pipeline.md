...
- **2026-05-21 follow-up (superseded — see 2026-05-25 below)** — Initial corpus-window PR (#25) keyed the floor on the most-recent **published** episode's date. Correct in steady state, but a failed fire didn't advance the floor, so the next successful fire would accumulate the missed window's briefs. Replaced 2026-05-25.
- **2026-05-25 correction** — `src/podcast/corpus.py:load_eligible_corpus` now applies a rolling 14-day window: `[run_date - 13 days, run_date]` inclusive, where `run_date` defaults to today in `EDITORIAL_TZ` (America/New_York). A failed publish does NOT widen the next run's window — missed windows are accepted losses per `feedback_accepted_single_day_miss.md`. Cold-start carve-out preserved (no episodes / empty episodes.json → full corpus).
