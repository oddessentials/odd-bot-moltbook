...
- **2026-05-21 follow-up** — `src/podcast/corpus.py:load_eligible_corpus` now filters briefs strictly newer than the most-recent published episode's `data/episodes.json` entry, selecting the floor by max episode `date` (not `episodeNo`). The default `episodes_path=EPISODES_PUBLIC_PATH` keeps production callers safe by default; the Episode 1 carve-out is preserved via the cold-start fallback (empty / missing episodes.json → no filter). Without this, the bi-weekly retune would have left each episode re-covering the full historical backlog.
